...
const ReservationModal = ({ show, onHide, room, onSuccess }) => {
  ...
  const calculateNights = () => {
    ...
    // Si es el mismo día (0 noches), contar como 1 día
  ...
  const handleSubmit = async (e) => {
    ...
    if (new Date(formData.checkIn) > new Date(formData.checkOut)) {
      setError('La fecha de check-out no puede ser anterior a la de check-in');
      return;
    }
        ...
        <Form onSubmit={handleSubmit}>
          ...
          {nights > 0 && (
            <div className="bg-primary text-white p-3 rounded mb-3">
              ...
              <Row>
                <Col>
                  <strong>{formData.checkIn === formData.checkOut ? 'Día:' : 'Noches:'}</strong> {nights}
                </Col>
                <Col>

...
app.post('/api/auth/login', 
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      { expiresIn: '24h' } // Reducido de 7d a 24h por seguridad
    );
...
app.get('/api/rooms/:id', validateId, async (req, res) => {
  try {
...
app.get('/api/rooms/:id/occupied-dates', validateId, async (req, res) => {
  try {
    const { Op } = require('sequelize');
...
app.put('/api/rooms/:id', authenticateToken, validateId, async (req, res) => {
  try {
...
app.post('/api/rooms', 
  authenticateToken, 
  validateRequired(['number', 'type', 'name', 'price', 'capacity']),
  validatePositiveNumber('price'),
  validatePositiveNumber('capacity'),
  async (req, res) => {
  try {
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      return res.status(400).json({ 
        message: `El número de habitación "${req.body.number}" ya existe. Por favor usa otro número.` 
      });
    }
...
app.delete('/api/rooms/:id', authenticateToken, validateId, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied' });
    }
...
app.post('/api/reservations', 
  authenticateToken,
  validateRequired(['roomId', 'checkIn', 'checkOut', 'adults']),
  validateDates,
  validatePositiveNumber('adults'),
  async (req, res) => {
  try {
    const {
      roomId,
...
app.put('/api/reservations/:id', authenticateToken, validateId, async (req, res) => {
  try {
    const { id } = req.params;
...
// Delete reservation
app.delete('/api/reservations/:id', authenticateToken, validateId, async (req, res) => {
  try {
    const { id } = req.params;
...
app.get('/api/reservations/:id', authenticateToken, validateId, async (req, res) => {
  try {
    const { id } = req.params;
...
app.post('/api/payments', authenticateToken, async (req, res) => {
  try {
    ...
    // Update reservation payment status and method
    reservation.paymentStatus = 'paid';
    reservation.paymentMethod = paymentMethodValue;
    await reservation.save();
...
app.get('/api/users/:id', authenticateToken, validateId, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin role required.' });
    }
...
app.post('/api/users', 
  authenticateToken,
  validateRequired(['firstName', 'lastName', 'email', 'password']),
  validateEmail,
  validatePassword,
  async (req, res) => {
  try {
...
app.put('/api/users/:id', authenticateToken, validateId, validateEmail, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied' });
    }
...
// Error handling
// Global error handler (debe ir al final, después de todas las rutas)

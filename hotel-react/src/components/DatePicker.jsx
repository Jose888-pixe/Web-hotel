...
const DatePicker = ({ checkInDate, checkOutDate, onCheckInChange, onCheckOutChange, occupiedDates = [], minDate }) => {
  ...
  useEffect(() => {
    console.log('🗓️ DatePicker received occupiedDates:', occupiedDates);
    console.log('🗓️ Total occupied dates:', occupiedDates.length);
  }, [occupiedDates]);

  useEffect(() => {
    console.log('🗓️ Check-in date:', checkInDate);
    console.log('🗓️ Check-out date:', checkOutDate);
  ...
  const handleDateClick = (day) => {
    ...
    if (dateStr === checkInDate) {
      console.log('🗑️ Clearing check-in');
      onCheckInChange('');
      onCheckOutChange('');
      return;
    }
    ...
    if (!checkInDate) {
      console.log('✅ Setting check-in:', dateStr);
    ...
    console.log('✅ Setting check-out:', dateStr);
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`calendar-day ${isOccupied ? 'occupied' : ''} ${isDisabled ? 'disabled' : ''} ${isCheckIn ? 'check-in' : ''} ${isCheckOut ? 'check-out' : ''} ${inRange && !isCheckIn && !isCheckOut ? 'in-range' : ''}`}
